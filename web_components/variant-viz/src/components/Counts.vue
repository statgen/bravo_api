  ...
  <div class="card shadow-sm" style="min-width: 420px">
    <div class="card-body">
      <div v-for="count in counts" class="container-fluid">
        <div class="row">
          <div class="col-sm-7 text-left text-truncate">{{ count.name }}</div>
          <div class="col-sm-5 text-right">{{ count.value }}</div>
        </div>
  ...
  computed: {
    counts: function() {
      return [
        ...
        { name: "Alternate allele count", value: this.variant.allele_count.toLocaleString(), percent: 100 * this.variant.allele_count / (2 * this.n_samples), max: 2 * this.n_samples },
        { name: "Alternate allele frequency", value: this.variant.allele_freq.toPrecision(5), percent: 100 * this.variant.allele_freq, max: 1.0 },
